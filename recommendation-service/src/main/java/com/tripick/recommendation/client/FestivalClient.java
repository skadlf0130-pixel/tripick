package com.tripick.recommendation.client;

import com.tripick.recommendation.client.dto.FestivalDetailEnvelope;
import com.tripick.recommendation.client.dto.FestivalListEnvelope;
import com.tripick.recommendation.client.dto.TravelSpotDetailEnvelope;
import com.tripick.recommendation.client.dto.TravelSpotListEnvelope;
import org.springframework.cloud.openfeign.FeignClient;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestParam;

/**
 * [recommendation-service] festival-service 동기 호출 Feign 클라이언트
 * AI 추천 생성 시 축제/여행지 후보 목록 조회 및 상세 정보 조회에 사용 (즉시 응답이 필요한 동기 통신)
 * 이벤트성 후처리는 Kafka 사용, 데이터 조회는 Feign 사용 원칙
 */
@FeignClient(name = "festival-service", url = "${feign.festival-service.url:http://localhost:8082}")
public interface FestivalClient {

    @GetMapping("/api/festivals")
    FestivalListEnvelope getFestivals(
            @RequestParam(value = "region", required = false) String region,
            @RequestParam(value = "page", defaultValue = "0") int page,
            @RequestParam(value = "size", defaultValue = "20") int size);

    @GetMapping("/api/festivals/{id}")
    FestivalDetailEnvelope getFestival(@PathVariable("id") Long id);

    @GetMapping("/api/travel-spots")
    TravelSpotListEnvelope getTravelSpots(
            @RequestParam(value = "region", required = false) String region,
            @RequestParam(value = "page", defaultValue = "0") int page,
            @RequestParam(value = "size", defaultValue = "20") int size);

    @GetMapping("/api/travel-spots/{id}")
    TravelSpotDetailEnvelope getTravelSpot(@PathVariable("id") Long id);
}
